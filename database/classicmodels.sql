...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Jan 17, 2017 at 11:06 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`%` PROCEDURE `get_daily_report`(IN `date` DATE, IN `act` VARCHAR(50))
BEGIN
	IF (SELECT `Day` FROM `daily_reports` WHERE `Day` = date LIMIT 1) THEN
		SELECT `customers`, `payments`, `orders`, `Action` from `daily_reports` where `Day` = date and `Action` = act;
	ELSE
		SELECT 'No result';
	END IF;
END$$
...
(500, 'test', 'undefined', 'undefined', 'undefined', '', NULL, '', NULL, NULL, '', NULL, NULL);

--
-- Triggers `customers`
--
DELIMITER $$
CREATE TRIGGER `customers_count_add` AFTER INSERT ON `customers`
 FOR EACH ROW BEGIN
	set @current_day = CURRENT_DATE();
    
    IF NOT EXISTS (SELECT `Day` from `daily_reports` 
                   where `Day` = @current_day and `Action` = 'Add')
   
    	THEN INSERT INTO `daily_reports` VALUES(CURRENT_DATE(),0,0,0,'Add');
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `customers_count_remove` AFTER DELETE ON `customers`
 FOR EACH ROW BEGIN
	set @current_day = CURRENT_DATE();
    ...
    IF NOT EXISTS (SELECT `Day` from `daily_reports` 
    	...
    	THEN INSERT INTO `daily_reports` VALUES(CURRENT_DATE(),0,0,0,'Remove');
    ...
    SET `customers` = `customers`+1,
    	`Action` = 'Remove'
    WHERE `Day` = @current_day AND `Action` = 'Remove';
 
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `customers_count_update` AFTER UPDATE ON `customers`
 FOR EACH ROW BEGIN
	set @current_day = CURRENT_DATE();
    
    IF NOT EXISTS (SELECT `Day` from `daily_reports` 
                   where `Day` = @current_day and `Action` = 'Update')
   
    	THEN INSERT INTO `daily_reports` VALUES(CURRENT_DATE(),0,0,0,'Update');
...
INSERT INTO `daily_reports` (`Day`, `customers`, `orders`, `payments`, `Action`) VALUES
('2017-01-12', 5, 1, 1, 'Remove'),
('2017-01-12', 2, 2, 1, 'Add'),
('2017-01-12', 1, 1, 1, 'Update'),
('2017-01-13', 1, 0, 0, 'Remove'),
('2017-01-14', 2, 0, 0, 'Update'),
('2017-01-14', 0, 1, 1, 'Add'),
('2017-01-15', 20, 0, 0, 'Add'),
('2017-01-17', 6, 0, 0, 'Add'),
('2017-01-17', 4, 0, 0, 'Update');
...
(0, '2017-01-14', '2017-01-14', '2017-01-14', '', NULL, 189),
...
--
DELIMITER $$
CREATE TRIGGER `orders_count_add` AFTER INSERT ON `orders`
 FOR EACH ROW BEGIN
	set @current_day = CURRENT_DATE();
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `orders_count_remove` AFTER DELETE ON `orders`
 FOR EACH ROW BEGIN
	set @current_day = CURRENT_DATE();
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `orders_count_update` AFTER UPDATE ON `orders`
 FOR EACH ROW BEGIN
	set @current_day = CURRENT_DATE();
...
--
DELIMITER $$
CREATE TRIGGER `payments_count_add` AFTER INSERT ON `payments`
 FOR EACH ROW BEGIN
	set @current_day = CURRENT_DATE();
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `payments_count_remove` AFTER DELETE ON `payments`
 FOR EACH ROW BEGIN
	set @current_day = CURRENT_DATE();
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `payments_count_update` AFTER UPDATE ON `payments`
 FOR EACH ROW BEGIN
	set @current_day = CURRENT_DATE();
...
--
-- Stand-in structure for view `total_product_sales`
--
CREATE TABLE IF NOT EXISTS `total_product_sales` (
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `customerorders` AS select `d`.`orderNumber` AS `orderNumber`,`c`.`customerName` AS `customerName`,sum((`d`.`quantityOrdered` * `d`.`priceEach`)) AS `total` from ((`orderdetails` `d` join `orders` `o` on((`o`.`orderNumber` = `d`.`orderNumber`))) join `customers` `c` on((`c`.`customerNumber` = `o`.`customerNumber`))) group by `d`.`orderNumber` order by sum((`d`.`quantityOrdered` * `d`.`priceEach`)) desc;

-- --------------------------------------------------------

--
-- Structure for view `total_product_sales`
--
DROP TABLE IF EXISTS `total_product_sales`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `total_product_sales` AS select `products`.`productName` AS `productName`,`products`.`productCode` AS `productCode`,(select sum((`orderdetails`.`quantityOrdered` * `orderdetails`.`priceEach`)) from `orderdetails` where (`orderdetails`.`productCode` = `products`.`productCode`)) AS `totalSales` from (`products` join `orderdetails` on((`products`.`productCode` = `orderdetails`.`productCode`))) group by `products`.`productName`;
